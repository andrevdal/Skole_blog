...
router.post(
	...
	async (req, res, next) => {
		...
		if (type === "Basic") {
			...
			return next(
				createError(400, "Only Basic authentification allowed")
			);
		try {
			await user.save();
...
router.get("/user", restrict, RateLimit(limitOptions), (req, res) =>
	res.jsonp(req.user)
);
	...
	async (req, res, next) => {
		...
		if (user) {
			if (req.body.keep === true) {
				const archive_user = await User.findOne({
					username: "archive",
				});
				try {
		...
		if (req.params.user) {
			...
			if (req.params.id) {
				...
				if (blog) {
					...
					return next(
						createError(404, "No blogs from this user found")
					);
			}
		} else {
			res.status(200).jsonp(
...
router.get("/", RateLimit(limitOptions), (_req, res) =>
	res.jsonp(
		router.stack
			.filter((r) => r.route)
			.map((r) => {
